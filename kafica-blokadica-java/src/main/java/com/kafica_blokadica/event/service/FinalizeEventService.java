...
public class FinalizeEventService {
    ...
    public FinalizeEventResponse processFinalization(Long eventId, FinalizionMethod method) {
        ...
        TimeOption bestTimeOption = timeOptions.stream().filter(timeOption -> timeOption.getId().equals(bestTimeId)).findFirst()
                .orElse(null);
        PlaceOption bestPlaceOption = placeOptions.stream().filter(placeOption -> placeOption.getId().equals(bestPlaceId)).findFirst()
                .orElse(null);
        ...
        String actionString = (event.getStatus() == EventStatus.CANCELLED) ? "canceled" : "finalized";


        //// moram srediti slanje notifikacija
        messagingTemplate.convertAndSend("/topic/events/" + eventId,
                new VotesUpdatedMessage(action, eventId, event.getCreatorUserId(), event.getFinalizedAt()));


        try {
            List<Long> participantIds = eventParticipantRepository.findAllByEventId(eventId)
                    .stream()
                    .map(EventParticipant::getUserId)
                    .toList();

            Map<String, Object> notificationData = actionString.equals("finalized")
                    ? Map.of(
                    "location", bestPlaceOption.getName(),
                    "time", bestTimeOption.getStartsAt().toString()
            )
                    : Map.of();
            ...
            pushNotificationService.sendToUsers(
                    participantIds,
                    "Event " + actionString,
                    "Event: " + event.getTitle() + " " + actionString,
                    notificationData
            );
        } catch (Exception e) {
            System.out.println("Failed to send push notifications: " + e.getMessage());
        }
